...
def generate_data(num_steps):
    ...
    for _ in range(1, num_steps):
        acc = 0.1 + np.random.normal(scale=0.5)
        accelerations.append(acc)
        # dr_acc.append(0.1 + np.random.normal(scale=0.5) + np.random.normal(scale=2))
        dr_acc.append(acc + np.random.normal(scale=1))
        true_states.append(true_state_transition(true_states[-1], velocities[-1], accelerations[-1], 0))
        dead_reckoning.append(true_state_transition(dead_reckoning[-1], dr_vel[-1], dr_acc[-1], 0))
        velocities.append(true_velocity_transition(velocities[-1], accelerations[-1], 0))
        dr_vel.append(true_velocity_transition(velocities[-1], dr_acc[-1], np.random.normal(scale=0.0)))
        measurements.append(observation_model(true_states[-1], np.random.normal(scale=0.1)))
    return dead_reckoning, true_states, measurements, dr_acc, dr_vel
...
def update(frame):
    ...
    particle_filter.predict(velocity=velocities[frame], acceleration=accelerations[frame], noise=np.random.normal(scale=0.9, size=NP))
    # 这种写法只生成一个噪声！若所有的粒子加的噪声都是相同的，就相当于只使用了一个粒子！
    # particle_filter.predict(velocity=velocities[frame], acceleration=accelerations[frame], noise=np.random.normal(scale=0.9))
    ##################################################################################
...
dead_reckoning, true_states, measurements, accelerations, velocities = generate_data(num_steps)
